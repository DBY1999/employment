...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        ...
        elif "browseduration"  in gerenjianli.getallcolumn(gerenjianli,gerenjianli):
...
#分类列表
def gerenjianli_lists(request):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":[]}
        msg['data'],_,_,_,_  = gerenjianli.page(gerenjianli, gerenjianli, {})
        return JsonResponse(msg, encoder=CustomJsonEncoder)

def gerenjianli_query(request):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        try:
            query_result = gerenjianli.objects.filter(**request.session.get("req_dict")).values()
...
def gerenjianli_list(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        q = Q()
        msg['data']['list'], msg['data']['currPage'], msg['data']['totalPage'], msg['data']['total'], \
        msg['data']['pageSize']  = gerenjianli.page(gerenjianli, gerenjianli, req_dict, request, q)
        return JsonResponse(msg, encoder=CustomJsonEncoder)
...
def gerenjianli_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= gerenjianli.createbyreq(gerenjianli,gerenjianli, req_dict)
...
def gerenjianli_update(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if 'clicktime' in req_dict.keys() and req_dict['clicktime']=="None":
            del req_dict['clicktime']
        if req_dict.get("mima") and "mima" not in gerenjianli.getallcolumn(gerenjianli,gerenjianli) :
            del req_dict["mima"]
        if req_dict.get("password") and "password" not in gerenjianli.getallcolumn(gerenjianli,gerenjianli) :
            ...
            try:
                for row in range(1, rows):
                    ...
                    if '.0' in str(row_values[0]):
                        req_dict['zhanghao'] = str(row_values[0]).split('.')[0]
                    elif str(row_values[0]) != '':
                        req_dict['zhanghao'] = row_values[0]
                    else:
                        req_dict['zhanghao'] = None
                    if '.0' in str(row_values[1]):
                        req_dict['xingming'] = str(row_values[1]).split('.')[0]
                    elif str(row_values[1]) != '':
                        req_dict['xingming'] = row_values[1]
                    else:
                        req_dict['xingming'] = None
                    if '.0' in str(row_values[2]):
                        req_dict['xingbie'] = str(row_values[2]).split('.')[0]
                    elif str(row_values[2]) != '':
                        req_dict['xingbie'] = row_values[2]
                    else:
                        req_dict['xingbie'] = None
                    if '.0' in str(row_values[3]):
                        req_dict['shouji'] = str(row_values[3]).split('.')[0]
                    elif str(row_values[3]) != '':
                        req_dict['shouji'] = row_values[3]
                    else:
                        req_dict['shouji'] = None
                    if '.0' in str(row_values[4]):
                        req_dict['zhuanye'] = str(row_values[4]).split('.')[0]
                    elif str(row_values[4]) != '':
                        req_dict['zhuanye'] = row_values[4]
                    else:
                        req_dict['zhuanye'] = None
                    if '.0' in str(row_values[5]):
                        req_dict['xueli'] = str(row_values[5]).split('.')[0]
                    elif str(row_values[5]) != '':
                        req_dict['xueli'] = row_values[5]
                    else:
                        req_dict['xueli'] = None
                    if '.0' in str(row_values[6]):
                        req_dict['qiuzhiyixiang'] = str(row_values[6]).split('.')[0]
                    elif str(row_values[6]) != '':
                        req_dict['qiuzhiyixiang'] = row_values[6]
                    else:
                        req_dict['qiuzhiyixiang'] = None
                    if '.0' in str(row_values[7]):
                        req_dict['jiaoyubeijing'] = str(row_values[7]).split('.')[0]
                    elif str(row_values[7]) != '':
                        req_dict['jiaoyubeijing'] = row_values[7]
                    else:
                        req_dict['jiaoyubeijing'] = None
                    if '.0' in str(row_values[8]):
                        req_dict['yixiangchengshi'] = str(row_values[8]).split('.')[0]
                    elif str(row_values[8]) != '':
                        req_dict['yixiangchengshi'] = row_values[8]
                    else:
                        req_dict['yixiangchengshi'] = None
                    if '.0' in str(row_values[9]):
                        req_dict['xinziyaoqiu'] = str(row_values[9]).split('.')[0]
                    elif str(row_values[9]) != '':
                        req_dict['xinziyaoqiu'] = row_values[9]
                    else:
                        req_dict['xinziyaoqiu'] = None
                    if '.0' in str(row_values[10]):
                        req_dict['gongzuojingli'] = str(row_values[10]).split('.')[0]
                    elif str(row_values[10]) != '':
                        req_dict['gongzuojingli'] = row_values[10]
                    else:
                        req_dict['gongzuojingli'] = None
