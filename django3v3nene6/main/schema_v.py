            ...
            while True:
                ...
                if WebSocket.has_messages():
                    ...
                    if client_msg is None:
                        continue
                    # 设置发送前端的数据
                    messages = f"user_id:{user_id}:{client_msg.decode('utf-8')}"
                    if to_id and to_id in connected_clients:
                        connected_clients[to_id].send(messages.encode('utf-8'))
                    else:
...
def schemaName_spider(request, tableName):
    if request.method in ["POST", "GET"]:
        ...
        # Linux
        cmd = "cd /yykj/python/9999/spider${spiderSchemaName} && scrapy crawl "+tableName+"Spider -a databaseName=django3v3nene6"
        # Windows
        # cmd = "cd C:\\test1\\spider && scrapy crawl " + tableName + "Spider -a databaseName=django3v3nene6"
        os.system(cmd)
