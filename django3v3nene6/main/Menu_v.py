...
def menu_register(request):
    if request.method in ["POST", "GET"]:
        ...
        error = menu.createbyreq(menu, menu, req_dict)
...
def menu_login(request):
    if request.method in ["POST", "GET"]:
        msg = {'code': normal_code, "msg": mes.normal_code}
        req_dict = request.session.get("req_dict")
        datas = menu.getbyparams(menu, menu, req_dict)
        if not datas:
        ...
        return Auth.authenticate(Auth, menu, req_dict)


def menu_logout(request):
    if request.method in ["POST", "GET"]:
        msg = {
            "msg": "登出成功",
            "code": 0
...
def menu_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            __loginUserColumn__= menu.__loginUserColumn__
        except:
        ...
        records=menu.getbyparams(menu, menu, recordsParam)
        if len(records)<1:
            msg['code'] = 400
            msg['msg'] = '用户不存在'
            return JsonResponse(msg, encoder=CustomJsonEncoder)

        eval('''menu.objects.filter({}='{}').update({}='{}')'''.format(username_str,username,password_str,init_pwd))
        
        return JsonResponse(msg, encoder=CustomJsonEncoder)



def menu_session(request):
    '''
    '''
    if request.method in ["POST", "GET"]:
        ...
        msg['data']  = menu.getbyparams(menu, menu, req_dict)[0]
...
def menu_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        msg['data']['pageSize']  =menu.page(menu, menu, req_dict, request)
        return JsonResponse(msg, encoder=CustomJsonEncoder)
...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":{"currPage":1,"totalPage":1,"total":1,"pageSize":10,"list":[]}}
        req_dict = request.session.get("req_dict")
        if "clicknum"  in menu.getallcolumn(menu,menu):
...
def menu_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= menu.createbyreq(menu,menu, req_dict)
...
def menu_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __browseClick__=="是"  and  "clicknum"  in menu.getallcolumn(menu,menu):
            try:
...
def menu_update(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if req_dict.get("mima") and "mima" not in menu.getallcolumn(menu,menu) :
            del req_dict["mima"]
        if req_dict.get("password") and "password" not in menu.getallcolumn(menu,menu) :
        ...
        if file_type in ['xlsx', 'xls']:
            ...
            try:
                for row in range(1, rows):
                    ...
                    req_dict = {}
                    menu.createbyreq(menu, menu, req_dict)
...
def menu_security(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT * FROM menu where ='{0}'".format(req_dict['username'])
        record = None
