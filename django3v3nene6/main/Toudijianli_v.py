...
def toudijianli_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        records=toudijianli.getbyparams(toudijianli, toudijianli, recordsParam)
        if len(records)<1:
            msg['code'] = 400
            msg['msg'] = '用户不存在'
            return JsonResponse(msg, encoder=CustomJsonEncoder)
...
def toudijianli_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        msg['data']['pageSize']  =toudijianli.page(toudijianli, toudijianli, req_dict, request)
        return JsonResponse(msg, encoder=CustomJsonEncoder)
...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        ...
        elif "browseduration"  in toudijianli.getallcolumn(toudijianli,toudijianli):
...
#分类列表
def toudijianli_lists(request):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":[]}
        msg['data'],_,_,_,_  = toudijianli.page(toudijianli, toudijianli, {})
        return JsonResponse(msg, encoder=CustomJsonEncoder)

def toudijianli_query(request):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        try:
            query_result = toudijianli.objects.filter(**request.session.get("req_dict")).values()
...
def toudijianli_list(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        q = Q()
        msg['data']['list'], msg['data']['currPage'], msg['data']['totalPage'], msg['data']['total'], \
        msg['data']['pageSize']  = toudijianli.page(toudijianli, toudijianli, req_dict, request, q)
        return JsonResponse(msg, encoder=CustomJsonEncoder)
...
def toudijianli_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= toudijianli.createbyreq(toudijianli,toudijianli, req_dict)
...
def toudijianli_update(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if 'clicktime' in req_dict.keys() and req_dict['clicktime']=="None":
            del req_dict['clicktime']
        if req_dict.get("mima") and "mima" not in toudijianli.getallcolumn(toudijianli,toudijianli) :
            del req_dict["mima"]
        if req_dict.get("password") and "password" not in toudijianli.getallcolumn(toudijianli,toudijianli) :
...
def toudijianli_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, ROUND(sum({1}),2) total FROM toudijianli {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d')".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, ROUND(sum({1}),2) total FROM toudijianli {2} GROUP BY DATE_FORMAT({0}, '%Y-%m')".format(xColumnName, yColumnName, where, '%Y-%m')
        ...
        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, ROUND(sum({1}),2) total FROM toudijianli {2} GROUP BY DATE_FORMAT({0}, '%Y')".format(xColumnName, yColumnName, where, '%Y')
...
def toudijianli_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, ROUND(sum({1}),2) AS total FROM toudijianli {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
# （按值统计）时间统计类型(多)
def toudijianli_valueMul(request, xColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = ''
            if timeStatType == '日':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, ROUND(sum({1}),2) total FROM toudijianli {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d')".format(xColumnName, item, where, '%Y-%m-%d')

            if timeStatType == '月':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, ROUND(sum({1}),2) total FROM toudijianli {2} GROUP BY DATE_FORMAT({0}, '%Y-%m')".format(xColumnName, item, where, '%Y-%m')

            if timeStatType == '季':
                sql = "SELECT CONCAT(YEAR(MIN({0})), '-Q', QUARTER(MIN({0}))) {0}, sum({1}) total FROM toudijianli {2} GROUP BY YEAR({0}), QUARTER({0})".format(xColumnName, item, where)

            if timeStatType == '年':
                sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, ROUND(sum({1}),2) total FROM toudijianli {2} GROUP BY DATE_FORMAT({0}, '%Y')".format(xColumnName, item, where, '%Y')
...
# （按值统计(多)）
def toudijianli_o_valueMul(request, xColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        ...
        if decode_str['tablename'] == 'qiye':
            where = where + " and qiyezhanghao ='{0}' ".format(decode_str['params']['qiyezhanghao'])
        for item in req_dict['yColumnNameMul'].split(','):
            sql = "SELECT {0}, ROUND(sum({1}),2) AS total FROM toudijianli {2} GROUP BY {0}".format(xColumnName, item, where)
            L = []
            cursor = connection.cursor()
            cursor.execute(sql)
            desc = cursor.description
...
def toudijianli_group(request, columnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        ...
        if decode_str['tablename'] == 'qiye':
            where = where + " and qiyezhanghao ='{0}' ".format(decode_str['params']['qiyezhanghao'])
        sql = "SELECT COUNT(*) AS total, " + columnName + " FROM toudijianli " + where + " GROUP BY " + columnName
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def toudijianli_typeStat_xingbie_zhiweimingcheng(request):
    '''
    新增分组统计接口
    '''
    if request.method in ["POST", "GET"]:
        ...
        sql = """select 
            zhiweimingcheng,
            sum(case when xingbie in ('男') then 1 else 0 end) as 男,            sum(case when xingbie in ('女') then 1 else 0 end) as 女            from toudijianli
            {where}
            group by zhiweimingcheng
            """.format(where=where)
...
def toudijianli_shBatch(request):
    '''
    批量审核
    '''
    if request.method in ["POST", "GET"]:
        ...
        for id in req_dict.get("ids"):
            data =toudijianli.getbyid(toudijianli,toudijianli, int(id))
            if len(data)>0:
                data=data[0]
                data['sfsh'] = req_dict.get("sfsh")
                data['shhf'] = req_dict.get("shhf")
                toudijianli.updatebyparams(toudijianli, toudijianli, data)
