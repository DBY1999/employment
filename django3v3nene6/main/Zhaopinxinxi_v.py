...
#分类列表
def zhaopinxinxi_lists(request):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":[]}
        msg['data'],_,_,_,_  = zhaopinxinxi.page(zhaopinxinxi, zhaopinxinxi, {})
        return JsonResponse(msg, encoder=CustomJsonEncoder)

def zhaopinxinxi_query(request):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        try:
            query_result = zhaopinxinxi.objects.filter(**request.session.get("req_dict")).values()
...
def zhaopinxinxi_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= zhaopinxinxi.createbyreq(zhaopinxinxi,zhaopinxinxi, req_dict)
...
def zhaopinxinxi_shBatch(request):
    '''
    批量审核
    '''
    if request.method in ["POST", "GET"]:
        ...
        for id in req_dict.get("ids"):
            data =zhaopinxinxi.getbyid(zhaopinxinxi,zhaopinxinxi, int(id))
            if len(data)>0:
                data=data[0]
                data['sfsh'] = req_dict.get("sfsh")
                data['shhf'] = req_dict.get("shhf")
                zhaopinxinxi.updatebyparams(zhaopinxinxi, zhaopinxinxi, data)
